...
netsh advfirewall firewall show rule name="巨神兵API辅助平台AI平台" >nul 2>&1
if %errorlevel% neq 0 (
    echo [警告] 防火墙规则不存在，尝试添加...
    netsh advfirewall firewall add rule name="巨神兵API辅助平台AI平台" dir=in action=allow protocol=tcp localport=8000 >nul 2>&1
    if %errorlevel% equ 0 (
