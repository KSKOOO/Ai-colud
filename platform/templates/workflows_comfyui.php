...
<head>
    ...
    <title>工作流编辑器 - 巨神兵AIAPI辅助平台</title>
    ...
    <script>
        ...
        const NODE_DEFINITIONS = {
            ...
            'GPTNode': {
                ...
                outputs: ['TEXT'],
                widgets: [
                    { name: 'provider', type: 'combo', options: 'DYNAMIC_PROVIDERS', default: '', label: '提供商' },
        ...
        function loadAllProvidersForWorkflow(callback) {
            $.ajax({
                url: 'api/providers_handler.php?action=get_providers&enabled=1',
                method: 'GET',
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.success && response.data) {
                        ...
                        response.data.forEach(provider => {
                            ...
                            if (!availableProviders[providerType]) {
                            ...
                            availableProviders[providerType].models = [...new Set([
                                ...availableProviders[providerType].models,
        ...
        function getAvailableModelsForNodes(provider) {
            ...
            if (provider && availableProviders[provider]) {
                ...
                const firstProviderKey = Object.keys(availableProviders)[0];
                if (firstProviderKey) {
                    models = availableProviders[firstProviderKey].models;
                }
            }
            // 确保返回数组
            ...
            if (def.widgets) {
                def.widgets.forEach(w => {
                    ...
                    if (w.options === 'DYNAMIC_PROVIDERS') {
                        ...
                        const providerOptions = getAvailableProvidersForNodes();
                        const provider = providerWidget ? (node.widgets['provider'] || providerOptions[0]?.value || '') : '';
            ...
            if (def.widgets) {
                def.widgets.forEach(w => {
                    ...
                    if (w.type === 'textarea') {
                        ...
                        if (options === 'DYNAMIC_MODELS') {
                            // 获取当前选中的提供商
                            const providerValue = node.widgets['provider'] || getAvailableProvidersForNodes()[0]?.value || '';
