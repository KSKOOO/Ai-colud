...
<head>
    ...
    <title>AI场景演示中心 - <?php echo $config['app']['name'] ?? '巨神兵AIAPI辅助平台'; ?></title>
...
<body>
        ...
        <div class="demo-content">
            ...
            <div class="demo-body">
                ...
                <div class="input-section">
                    ...
                    <!-- 视频剪辑微调参数面板 -->
                    <div id="fineTunePanel" style="display: none; margin: 16px 0; padding: 16px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px; color: #1e293b; font-weight: 600;">
                            <i class="fas fa-sliders-h"></i> 剪辑微调参数
                            <span style="font-size: 12px; color: #64748b; font-weight: normal;">（可选，AI将根据这些参数优化剪辑效果）</span>
                        </div>
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px;">
                            <div>
                                <label style="display: block; font-size: 13px; color: #64748b; margin-bottom: 4px;">剪辑风格</label>
                                <select id="clipStyle" style="width: 100%; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 14px;">
                                    <option value="auto">智能自动</option>
                                    <option value="fast">快节奏（适合抖音）</option>
                                    <option value="slow">慢节奏（适合Vlog）</option>
                                    <option value="cinematic">电影感（专业级）</option>
                                </select>
                            </div>
                            <div>
                                <label style="display: block; font-size: 13px; color: #64748b; margin-bottom: 4px;">转场效果</label>
                                <select id="transitionType" style="width: 100%; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 14px;">
                                    <option value="fade">淡入淡出</option>
                                    <option value="slide">滑动切换</option>
                                    <option value="zoom">缩放转场</option>
                                    <option value="none">无转场</option>
                                </select>
                            </div>
                            <div>
                                <label style="display: block; font-size: 13px; color: #64748b; margin-bottom: 4px;">调色风格</label>
                                <select id="colorGrade" style="width: 100%; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 14px;">
                                    <option value="明亮">明亮清新</option>
                                    <option value="暖色">温暖色调</option>
                                    <option value="冷色">冷色调</option>
                                    <option value="电影感">电影质感</option>
                                    <option value="复古">复古风格</option>
                                </select>
                            </div>
                            <div>
                                <label style="display: block; font-size: 13px; color: #64748b; margin-bottom: 4px;">音频处理</label>
                                <select id="audioEnhance" style="width: 100%; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 14px;">
                                    <option value="loudnorm">智能音量平衡</option>
                                    <option value="volume_up">音量提升</option>
                                    <option value="denoise">降噪处理</option>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-top: 12px;">
                            <div>
                                <label style="display: block; font-size: 13px; color: #64748b; margin-bottom: 4px;">最小片段时长: <span id="minClipVal">3</span>秒</label>
                                <input type="range" id="minClipDuration" min="2" max="10" value="3" style="width: 100%;" oninput="document.getElementById('minClipVal').textContent=this.value">
                            </div>
                            <div>
                                <label style="display: block; font-size: 13px; color: #64748b; margin-bottom: 4px;">最大片段时长: <span id="maxClipVal">15</span>秒</label>
                                <input type="range" id="maxClipDuration" min="5" max="30" value="15" style="width: 100%;" oninput="document.getElementById('maxClipVal').textContent=this.value">
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function openDemo(type) {
            ...
            // 显示/隐藏视频剪辑微调面板
            const isVideoEdit = type === 'video-edit' || type === 'live-highlight';
            document.getElementById('fineTunePanel').style.display = isVideoEdit ? 'block' : 'none';
        ...
        function generateContent() {
            ...
            // 添加视频剪辑微调参数
            if (currentScenario === 'video-edit' || currentScenario === 'live-highlight') {
                const fineTuneParams = {
                    clip_style: document.getElementById('clipStyle').value,
                    transition_type: document.getElementById('transitionType').value,
                    color_grade: document.getElementById('colorGrade').value,
                    audio_enhance: document.getElementById('audioEnhance').value,
                    min_clip_duration: parseInt(document.getElementById('minClipDuration').value),
                    max_clip_duration: parseInt(document.getElementById('maxClipDuration').value)
                };
                formData.append('fine_tune_params', JSON.stringify(fineTuneParams));
            }
