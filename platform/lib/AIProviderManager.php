...
class AIProviderManager {
    ...
    const REMOVED_PROVIDER_TYPES = ['ollama'];
    ...
    public static function isRemovedProviderType($type) {
        return in_array((string)$type, self::REMOVED_PROVIDER_TYPES, true);
    }
    ...
    public function loadProviders() {
        ...
        $providersChanged = $this->removeLegacyProviders();
        $providersChanged = $this->ensureProviderIds() || $providersChanged;
        $providersChanged = $this->ensureDefaultProvider() || $providersChanged;
        if ($providersChanged) {
            $this->saveProviders();
        }
    ...
    private function removeLegacyProviders() {
        $changed = false;

        foreach ($this->providers as $id => $provider) {
            if (self::isRemovedProviderType($provider['type'] ?? '')) {
    ...
    private function ensureProviderIds() {
        $changed = false;
    ...
    private function ensureDefaultProvider() {
        $enabledIds = [];
        $defaultEnabledIds = [];
        ...
        $targetDefaultId = $defaultEnabledIds[0] ?? $enabledIds[0];
        $changed = false;

        foreach ($this->providers as $id => &$provider) {
            $shouldBeDefault = ($id === $targetDefaultId);
            if (($provider['is_default'] ?? false) !== $shouldBeDefault) {
                $provider['is_default'] = $shouldBeDefault;
                $changed = true;
            }
        }
    ...
    public function getProviderTypes() {
        return array_filter(
            self::PROVIDER_TYPES,
            function($info, $type) {
                return !self::isRemovedProviderType($type);
            },
    ...
    public function getImageProviders($onlyEnabled = false) {
        ...
        return array_filter($providers, function($p) {
            ...
            $imageSupportedTypes = ['openai', 'azure_openai', 'gemini', 'qwen', 'hunyuan', 'anthropic', 'zhipu', 'deepseek', 'moonshot', 'custom_openai', 'llamacpp', 'vllm', 'xinference', 'gpustack'];
    ...
    public function addProvider($data) {
        ...
        if (self::isRemovedProviderType($type)) {
            throw new Exception("Provider type disabled: {$type}");
        }
